  ...
  ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'pedagang_actions', {
      ...
      if data.current.value == 'menu_crafting' then
          ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'menu_crafting', {
                ...
                TriggerServerEvent('esx_pedagangjob:craftingCoktails', data2.current.value)
...
function OpenShopMenu(zone)
    ...
    ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'pedagang_shop', {
            title    = _U('shop'),
            elements = elements
        }, function(data, menu)
...
Citizen.CreateThread(function()
  while true do
    ...
    if IsControlJustReleased(0,  167) and IsJobTrue() and not ESX.UI.Menu.IsOpen('default', GetCurrentResourceName(), 'pedagang_actions') then
...
-----------------------
----- TELEPORTERS -----
AddEventHandler('esx_pedagangjob:teleportMarkers', function(position)
  ...
  while true do
    ...
    if IsJobTrue() then
        ...
        if IsControlJustReleased(0, 38) and isInPublicMarker then
          TriggerEvent('esx_pedagangjob:teleportMarkers', position)
        end
